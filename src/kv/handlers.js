...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env[env.DATABASE].get("proxySettings", { type: 'json' });
        warpConfigs = await env[env.DATABASE].get('warpConfigs', { type: 'json' });
    } catch (error) {
        console.log(error);
...
export async function updateDataset(request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env[env.DATABASE].get("proxySettings", { type: 'json' });
        } catch (error) {
            console.log(error);
